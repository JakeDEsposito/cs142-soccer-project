...
SoccerReg: $(O)/ui.o $(O)/ledge.o $(O)/player.o
	$(CC) $(CFLAGS11) $(O)/ui.o $(O)/ledge.o $(O)/player.o -o $(E)/SoccerReg
$(O)/ledge.o: $(PR)/ledge.cpp $(PU)/ledge.h $(PU)/player.h
	$(CC) $(CFLAGS11) -c $(PR)/ledge.cpp -o $(O)/ledge.o
$(O)/player.o: $(PR)/player.cpp $(PU)/player.h
	$(CC) $(CFLAGS11) -c $(PR)/player.cpp -o $(O)/player.o
$(O)/ui.o: $(PR)/ui.cpp $(PU)/ledge.h $(PU)/player.h
