...
bool searchView(Ledge & playerList, vector<Player> & p)
{
  ...
  while(true)
	...
	case 'E':
	  {
	    rExit=pExit=endProgram(playerList);
	    break;
	  }
	default:
	  {
...
int main()
{
  ...
  while (true)//main view
	...
	case 'N':
	  {
	    cout<<"WARNING: This will delete all players!!! Press [Y] to start a new season";
	    cout<<" or [N] to cancel:\t";
	    cin>>control;
	    cout<<endl;
	...
	case 'A':
	  {
	    ...
	    cout<<"Enter their year of birth:\t";
	    cin>>yob;
	    
	    cout<<"Enter [1] if they are registered or [0] if they are un-registered:\t";
	    cin>>temp;
	    if(temp.at(0)=='0')
	      regStat=false;
	    else if(temp.at(0)=='1')
	    ...
	    ledger.addPlayer(Player(fn, ln, yob, regStat, ledger.programYear));
	    break;
	  }
	case 'F':
	  {
	...
	case 'S':
	  {
	    if(saveLedger(ledger))
	      {
		cout<<"The ledger has been saved"<<endl;
	      }
	    break;
	  }
	case 'E':
	  {
	    bool temp=endProgram(ledger);
	    endP=temp;
	    break;
	  }
	default:
